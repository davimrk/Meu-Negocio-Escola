...
app.post("/api/lojas", (req, res) => {
  ...
  db.query(
    ...
    (erro, resultado) => {
      ...
      res.status(201).json({ id_loja: resultado.insertId });
    },
  );
});

app.put("/api/lojas/:id", (req, res) => {
  const { nome, descricao, horario_funcionamento, ativa, foto_logo } = req.body;
...
app.get("/cardapio/:idLoja", (req, res) => {
  const sql = `
    SELECT produto.id_produto, produto.nome, produto.descricao, produto.preco,
           produto.foto, categoria.nome AS categoria
    FROM Produto produto
    LEFT JOIN Categoria categoria ON produto.id_categoria = categoria.id_categoria
    WHERE produto.id_loja = ? AND produto.disponivel = 'sim'
    ORDER BY categoria.nome
  `;

  db.query(sql, [req.params.idLoja], (erro, dados) => {
    if (erro) {
      console.log(erro);
...
app.post("/api/produtos", (req, res) => {
  ...
  db.query(
    ...
    (erro, resultado) => {
      ...
      res.json({ mensagem: "Produto criado", id_produto: resultado.insertId });
    },
  );
});
...
app.get("/api/produtos", (req, res) => {
  ...
  let sql = `
    SELECT produto.id_produto, produto.id_loja, produto.id_categoria, produto.nome,
           produto.descricao, produto.preco, produto.foto,
           loja.nome AS loja, categoria.nome AS categoria
    FROM produto
...
app.get("/api/produtos/loja/:id_loja", (req, res) => {
  ...
  db.query(sql, [req.params.id_loja], (erro, resultado) => {
    if (erro) {
      console.log(erro);
      return res.status(500).json({ erro: "Erro ao buscar produtos da loja" });
    }
    res.json(resultado);
  });
});
...
app.get("/api/produtos/:id", (req, res) => {
  const sql = `
    SELECT produto.id_produto, produto.nome, produto.descricao,
           produto.preco, produto.foto, loja.nome AS loja
    FROM produto
...
function buscarOuCriarCarrinhoAberto(id_usuario, callback) {
  db.query(
    ...
    (erro, resultado) => {
      ...
      if (resultado.length > 0) return callback(null, resultado[0].id_carrinho);

      db.query(
        "INSERT INTO carrinho (id_usuario, data_criacao, statusC) VALUES (?, CURDATE(), 'aberto')",
        [id_usuario],
        (erro2, resultadoInsert) => {
...
app.get("/api/carrinho/:id_usuario", (req, res) => {
  db.query(
    ...
    (erro, resultadoCarrinho) => {
      ...
      db.query(
        `SELECT itemcarrinho.id_item_carrinho, itemcarrinho.quantidade,
                produto.id_produto, produto.nome, produto.preco, produto.foto,
      ...
      (erro2, resultado) => {
        ...
        if (resultado.length > 0) {
          db.query(
            ...
            [resultado[0].quantidade + qtd, resultado[0].id_item_carrinho],
            (erro3) => {
              if (erro3) {
                console.error(erro3);
                return res
...
app.post("/api/encomendas", (req, res) => {
  ...
  db.query(
    ...
    (erro, resultadoCarrinho) => {
      ...
      db.query(
        `SELECT itemcarrinho.id_produto, itemcarrinho.quantidade, produto.preco, produto.id_loja
         FROM itemcarrinho
         JOIN produto ON produto.id_produto = itemcarrinho.id_produto
         WHERE itemcarrinho.id_carrinho = ?`,
        [id_carrinho],
          ...
          function processarProximaLoja() {
            if (indiceLojaAtual >= idsLojas.length) {
              db.query(
                ...
                [id_carrinho],
                (erroFinaliza) => {
                  if (erroFinaliza) console.error(erroFinaliza);
            ...
            db.query(
              ...
              (erro3, resultadoEncomenda) => {
                ...
                db.query(
                  "INSERT INTO itemencomenda (id_encomenda, id_produto, quantidade, preco_unitario, subtotal) VALUES ?",
                  [valoresItens],
                  (erro4) => {
                    if (erro4) {
                      console.error(erro4);
                    ...
                    db.query(
                      "INSERT INTO retirada (id_encomenda, codigo_retirada, nome, data_retirada) VALUES (?, ?, ?, NULL)",
...
app.get("/api/encomendas/loja/:id_loja", (req, res) => {
  db.query(
    `SELECT DISTINCT encomenda.id_encomenda, encomenda.data, encomenda.status,
            encomenda.valor_total, encomenda.observacao,
            retirada.codigo_retirada, retirada.nome AS nome_retirada,
            retirada.data_retirada, usuario.nome AS nome_aluno
     FROM encomenda
    ...
    (erro, encomendas) => {
      ...
      db.query(
        `SELECT itemencomenda.id_encomenda, itemencomenda.quantidade,
                itemencomenda.preco_unitario, itemencomenda.subtotal, produto.nome
         FROM itemencomenda
         JOIN produto ON produto.id_produto = itemencomenda.id_produto
